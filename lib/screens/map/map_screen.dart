...
class MapScreen extends StatefulWidget {
  const MapScreen({Key? key, required this.onAddressFound}) : super(key: key);
...
class _MapScreenState extends State<MapScreen> {
  ...
  @override
  void initState() {
    loadCurrentLocation().then((LatLng currentPosition) {
      setState(() {
  ...
  setPoint(LatLng position) async {
    BitmapDescriptor icon = await getMarkerBitmap();
    setState(() {
      _markers = {
        Marker(
  ...
  Future<BitmapDescriptor> getMarkerBitmap() async {
    const size = 40;
