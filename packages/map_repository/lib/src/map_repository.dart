...
class MapRepository {
  ...
  MapRepository({required this.googleApiKey, required SafeWayAPI api})
      : _api = api;

  Future<List> autocomplete(String query) async {
    var response = await _dio
        .get('place/autocomplete/json?input=$query&key=$googleApiKey');
  ...
  Future searchPlaces(String query) async {
    var response = await _dio.get(
        'place/findplacefromtext/json?fields=geometry&inputtype=textquery&input=$query&key=$googleApiKey');
    return response.data;
  }

  Future getAddressByLocation(double latitude, double longitude) async {
    var response = await _dio
        .get('geocode/json?latlng=$latitude,$longitude&key=$googleApiKey');
    return response.data;
  }
  ...
  //     List pois =
  //         response.data['items'].map((item) => POI.fromMap(item)).toList();
